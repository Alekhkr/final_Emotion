...
@app.websocket("/api/stream_audio")
async def audio_stream_endpoint(websocket: WebSocket):
    """
    Phase 3: WebRTC/WebSocket continuous sliding-window streaming.
    Accepts 16kHz PCM audio chunks every 500ms, calculates live acoustic 
    panic indicators and simulated SVI, and streams back JSON results.
    """
    await websocket.accept()
    logger.info("Live audio streaming connection established.")
    try:
        while True:
            # Receive binary audio chunk (e.g., 500ms of PCM data)
            data = await websocket.receive_bytes()
            
            # TODO: Pass `data` to streaming Librosa/VAD pipeline
            # Mocking live SVI calculation for demonstration purposes
            import random
            live_svi = round(random.uniform(30.0, 85.0), 1)
            
            payload = {
                "event": "LIVE_SVI_UPDATE",
                "timestamp_ms": 500, # sliding window
                "svi_score": live_svi,
                "acoustic_panic": round(random.uniform(0.1, 0.9), 2),
                "is_speaking": True
            }
            await websocket.send_json(payload)
            
    except WebSocketDisconnect:
        logger.info("Live audio streaming disconnected.")
...
async def analyze(
    ...
    if llm_reasoning and "svi_score" in llm_reasoning:
        ...
        # Add explicit SIH psychological cues
        psych_cues = llm_reasoning.get("psychological_vulnerability_cues", [])
        for cue in psych_cues:
            if cue and cue.lower() != "none":
                formatted_llm_nodes.append({
                    "source": "Cognitive Psychological Profiling",
                    "sign": cue,
                    "type": "llm_reasoner"
                })
