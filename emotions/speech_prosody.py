...
def extract_prosody_features(
    ...
    # ========================================================
    # 5.5 TEAGER ENERGY OPERATOR (TEO) - INVOLUNTARY MICRO-TREMORS
    # ========================================================
    # TEO isolates non-linear stress in the vocal folds (adrenaline-induced tremors).
    teo = y[1:-1]**2 - y[:-2] * y[2:]
    teo_mean = float(np.mean(np.abs(teo))) if len(teo) > 0 else 0.0
    teo_std = float(np.std(teo)) if len(teo) > 0 else 0.0
    
    # Calculate a specific stress marker: High TEO variance compared to mean
    # indicates vocal cord tension typical of extreme panic/fear.
    teo_stress_marker = float(teo_std / (teo_mean + 1e-6))
    teo_score = min(1.0, teo_stress_marker / 2.5)
    ...
    # The new max-accuracy blend incorporates TEO for involuntary tremor detection
    acoustic_panic_index = float(
        0.20 * jitter_score
        + 0.20 * shimmer_score
        + 0.20 * pitch_instab_score
        + 0.15 * pause_anomaly_score
        + 0.25 * teo_score
    )
    ...
    if teo_score > 0.7:
        detected_signs.append(f"Vocal Micro-Tremors Detected (TEO Stress Marker) — involuntary adrenaline response")
